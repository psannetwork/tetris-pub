'use strict';
const { tetrominoes, srsKick } = require('./constants.js');
...
function getAllPlacements(board,p){
  ...
  for(let i = 0; i < 3; i++) {
    ...
    if(rotated) {
      ...
      if(!exists) achievableOrientations.push(rotated);
...
module.exports = {
  getPieceBlocks, isValidPosition, mergePiece, clearLines, hardDrop,
  evaluateBoard, getAllPlacements, moveLeft, moveRight, moveDown,
  moveRotateCW, moveRotateCCW, detectTSpin, clonePiece
};
