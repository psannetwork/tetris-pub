...
import { currentPiece, movePiece, rotatePiece, hardDrop, hold, addGarbageLines, isGameStarted } from './game.js';

let isKeyOperation = true;
    ...
    if (isGameStarted) {
      switch (event.code) {
        ...
        case CONFIG.keyBindings.hold:
          ...
          break;
        case "KeyG":
          if (CONFIG.debug.enableGarbage) addGarbageLines(1);
          break;
      }
    }
  }
...
function handleRepeatKeys() {
  const now = performance.now();
  for (const key in keys) {
    if (now - keys[key].startTime > 200 && now - keys[key].lastRepeat > 50) {
      keys[key].lastRepeat = now;
      if (isGameStarted) {
        switch (key) {
          case CONFIG.keyBindings.moveLeft:
            movePiece({ x: -1, y: 0 });
...
setInterval(handleRepeatKeys, 50);